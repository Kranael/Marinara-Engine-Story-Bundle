...
- Added a "Surprise Me" option to the Story Bundle scenario picker: a static first card with a dark gradient, a dice icon, and a "Default" badge that asks the AI to invent an immersive opening hook from the active characters and setting. GM's "Start Adventure" flow now shows the same scenario dialog (persona picker first, then scenario) for bundles with saved scenarios, steering the first GM turn toward the chosen saved or custom scenario instead of always improvising. Canceling either the persona picker or the scenario dialog aborts the whole flow — no game is created either way.
- Removed the required-field markers (red asterisks and inline "is required before export" hints) from the Story Bundle Metadata tab's Genre/Setting/Tone fields; Export still stays disabled with an explanatory tooltip until all three are filled in.
