...
- Added a working CONVO button to Story Bundles (editor header, gallery detail card, and gallery cards): it now starts the conversation directly via the same DirectInject pattern GM uses — creating a group chat (1:n) with every bundle character, the bundle's persona (or staying anonymous if none is set), first connection, and preset, tagging it with the bundle, and activating its lorebooks and agents — with no setup wizard involved. A bundle with no characters shows an error; otherwise the conversation starts immediately with all of its characters.
