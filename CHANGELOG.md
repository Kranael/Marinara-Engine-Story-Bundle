...
- Added a Game Config section to the Story Bundle Metadata tab: creators set the bundle's Genre, Setting, and Tone directly (all three required), with a "Generate from Lore" button that samples the bundle's currently assigned characters and lorebook entries and asks the configured AI connection to suggest all three at once. Export is disabled — with an explanatory tooltip — from the panel row, editor, and Gallery until all three are filled in and saved, so every exported `.storybundle` carries a deterministic game config instead of leaving end-users to fill in defaults.
...
- Added a Story Bundle Gallery: a new gallery button in the Story Bundles panel opens a full-screen card gallery (same layout as the Character Gallery) that displays each bundle's artwork, title, and safely sanitized HTML description. Selecting a card shows its detail with Play, Edit, Export, and Delete actions that reuse the existing Story Bundle flows; the gallery supports search, name/date sorting, and works on mobile with an inline detail view.
- Added a Custom Scenario option to the Story Bundle scenario picker: instead of choosing a saved scenario, you can describe the desired starting situation in your own words and the opening message is generated with AI (reusing the same narrator-guided generation behind the "/narrator" command) while a loading toast explains that the scenario is starting; the generated message becomes the roleplay's first message the same way a saved scenario's opening message does.
- Added New Bundle and Import buttons to the Story Bundle Gallery toolbar, mirroring the Character Library's layout and reusing the existing create and import flows.
- Added a working CONVO button to Story Bundles (editor header, gallery detail card, and gallery cards): it now starts the conversation directly via the same DirectInject pattern GM uses — creating a group chat (1:n) with the chosen characters, the bundle's persona (or staying anonymous if none is set), first connection, and preset, tagging it with the bundle, and activating its lorebooks and agents — with no setup wizard involved. Clicking CONVO opens a multi-select character picker (the bundle's first character pre-selected) so the player chooses one or more characters to message; a bundle with no characters shows an error instead.
- Added a "None — Stay anonymous" option to the Story Bundle editor's Personas tab, so a bundle can be explicitly configured to play without a persona.
- Added Game Mode fields to Story Bundles: an Assets tab for scoping which music, ambient, sound-effect, sprite, and background folders a bundle's game may use (same folder-selection model as the native "Adjust Game Assets for this Game" picker), and a Party/NPC toggle on each assigned character in the Characters tab. A new DirectInject bootstrapper lets a Bundle Card's "Start Adventure" button create a Game Mode session directly from the bundle's frozen setup metadata and party mapping, then prompts for the player's persona (defaulting to the bundle's own) before handing off to the game session — skipping the multi-step Game Setup Wizard.
- Enabled the GM button on Story Bundles (Gallery cards, Gallery detail card, and the editor header): it now starts the DirectInject flow — creating the game, running its world-setup pass, and asking which persona to play as — instead of showing "Coming soon". The editor's GM button uses the current unsaved draft, same as Play.
- Fixed DirectInject failing to create a game for any Story Bundle without an imported `gameConfig`: empty genre/setting/tone/difficulty were rejected by the server, now falling back to the same defaults the native Game Setup Wizard uses (Fantasy / A fantasy world / Heroic / Normal).
- Fixed the GM "Who are you?" modal appearing to freeze after clicking "Start Adventure": it now shows a real step-by-step progress bar (creating the game, applying settings, generating the world, with a live elapsed-seconds counter for the AI world-generation step) and closes automatically once the session is ready, instead of requiring a manual close that left the new game sitting unopened in the chat list. The persona picker also now lists every persona in your library — the bundle's own persona is only pre-selected as the default, never the only option.
- Fixed Story Bundle export failing with "Internal Server Error" for bundles with many characters carrying large avatars/sprites/galleries: the export response is now streamed field-by-field instead of built as a single JSON string, which could previously exceed Node's ~512 MB max string length and crash the whole export.
- Added a "Surprise Me" option to the Story Bundle scenario picker: a static first card with a dark gradient, a dice icon, and a "Default" badge that asks the AI to invent an immersive opening hook from the active characters and setting.
- Replaced GM's separate "Who are you?" persona picker and scenario dialog with a single combined "Start Adventure" wizard modal (StoryBundleGmStartModal): a Persona step, then — only for bundles with saved scenarios — a Scenario step, styled like the native chat setup wizards (dot stepper, Next/Back footer, Start Adventure only on the final step). The X close button and the Persona step's Cancel always abort without creating a game; the Scenario step's Back button only returns to Persona. Picking a saved or custom scenario steers the first GM turn toward that situation instead of always improvising.
- Removed the required-field markers (red asterisks and inline "is required before export" hints) from the Story Bundle Metadata tab's Genre/Setting/Tone fields, and removed the export gate entirely — Export now works from the panel row, editor, and Gallery even when Genre/Setting/Tone are left blank.
- RP Play now shows the same Persona-then-Scenario "Start Adventure" wizard as GM (StoryBundleRpStartModal) instead of jumping straight to a scenario-only dialog, asking who you're playing as before the bundle's saved scenarios (or Surprise Me / Custom Scenario). Canceling the Persona step, or closing the wizard (X), aborts without creating a chat.
- Swapped the Story Bundle editor's Assets tab icon from a music note to the same folder icon used everywhere else in the app for Game Assets (Settings' Game Assets section, the in-game Assets toolbar button).
...
- Replaced the Story Bundle Play button in the editor header, the Story Bundle Gallery detail card, and every Story Bundle Gallery card with the same Conversation, Roleplay, and Game mode buttons used on Home, labeled compactly as CONVO, RP, and GM. Roleplay keeps the exact existing Play behavior; Conversation and Game are visible but disabled placeholders for future Story Bundle modes. The Story Bundle Gallery cards use Home's colored mode-button styling. The Story Bundles panel row no longer shows a Play action, only Export and Delete.
- Renamed Story Bundle Intros to Scenarios: a Scenario is only a starting situation (title, opening message, and an optional picture with the same avatar crop widget used elsewhere) — there are no branches, paths, or choices. The Scenarios tab shows each scenario's picture as a compact thumbnail (falling back to an icon when none is set), and picking a scenario when playing a bundle now opens a visual card picker with the picture as the main artwork and the title overlaid at the bottom, instead of a plain stacked-button list. Existing bundles with saved intros migrate automatically and keep their content.
- Restyled the Story Bundles panel's toolbar to match the Characters panel: a segmented "Open Gallery" button (using the same Story Bundles icon as the TopBar) above icon-only New Bundle and Import buttons, replacing the previous single row of icon buttons.
- The Story Bundle editor's Assets tab now wears the same GM Settings purple frame and label as the Characters and Metadata tabs' Game Mode sections, instead of a plain untinted panel.
- The Story Bundle Gallery detail card's CONVO, RP, and GM buttons now use the same colored borders and backgrounds as the matching buttons on the Gallery's grid cards, instead of a plain untinted style. Edit Bundle, Export, and Delete are unchanged.

### Fixed

- Fixed Game Mode's NPC portrait lookup wrongly linking a generated NPC to an unrelated library character that shared a single word of its name (e.g. "Ichika Nakano" inheriting "Ichika Matsuda"'s avatar): the fuzzy fallback now compares full names only and never matches on single-word aliases, so a name collision on one word no longer borrows another character's portrait.
- Fixed Game Setup dropping the entire campaign plan (and falling back to HUD widgets only) whenever the AI generated a single over-long string: `openingSituation`, pressure-clock names/failures, faction names/goals/methods/secrets, quest seeds, and encounter principles are now truncated to their schema limits instead of rejected, so one verbose value can no longer discard the whole blueprint.
- Story Bundle characters assigned as NPCs (not party members) are no longer discarded when Story Bundle "Start Adventure" bootstraps a new Game Mode session: they're now carried into the game as library-linked tracked NPCs, and the GM prompt injects their full character card (personality, backstory, appearance, any custom system instructions, and their card's tags) in a new `<known_npcs>` block — with an explicit instruction that they are not party members, shouldn't be expected to take independent turns, and must stay in-character for their stated role (e.g. a student-tagged NPC won't suddenly deliver a director's or teacher's speech). Dynamically-invented NPCs the GM creates during play are unaffected and keep using the existing lightweight tracked-NPC summary.
- Fixed the game's own world-setup pass wiping out those library-linked NPCs the moment it ran: it now merges its own newly-invented NPCs in alongside the ones already tracked instead of replacing the whole list, and skips inventing a duplicate for a name that's already known.
- Fixed Story Bundle "Start Adventure" leaving the full Game Setup wizard stuck open (requiring a manual close) after the game was already created and ready to play — Start Adventure now clears the same "setup in progress" flag the wizard itself would have cleared, so the adventure begins immediately after the widget review/Start Game confirmation.
- Game Mode's chat settings "Party" section is now labeled "Characters" and gained a World Characters list showing every tracked NPC (dynamic or library-linked), with a way to stop tracking one, alongside the existing Persona and Party Characters management.
- Story Bundle export now runs every avatar, sprite, and gallery PNG/JPG through an asset optimization pass that re-encodes it to WebP (quality 80) before it goes into the ZIP, falling back to the original file whenever that isn't smaller or `sharp` isn't available on the platform. This is what actually shrinks large exports — a 35-character bundle that previously produced a 400+ MB `.storybundle` now exports at roughly a tenth of that size, with no changes needed on import (bundles restore avatars by filename regardless of extension).
- Story Bundle export and import now use a `.storybundle` ZIP archive instead of a base64-embedded JSON file: avatars, sprites, gallery images, and game assets are stored as their original binary files (compressed at maximum level) rather than base64 text, so exporting bundles with many or large character images no longer risks exceeding Node's string-length limits. The Story Bundle import dialog now only accepts `.storybundle` archives — the legacy `.marinara.json` envelope import path, along with its embedded-content choice and missing-agents install prompt, has been removed from the dialog.
...
- Fixed Story Bundle Play navigating into the new roleplay before the chosen scenario's opening message had actually been inserted, which on mobile could leave the chat looking empty until a manual refresh; Play now waits for the opening message (or the generated Custom Scenario message) to be ready before entering the chat, while the existing Play button spinner keeps showing that setup is still in progress.
- Story Bundle images (bundle covers and scenario pictures) are now included in backups and profile exports/imports, and are deleted from disk when a bundle is deleted or its picture is replaced or removed — previously they were silently excluded from every backup and leaked on disk indefinitely.
- Fixed the Story Bundle editor silently discarding unsaved edits to every other field (name, description, tags, characters, personas, lorebooks, presets, agents, party assignment, scenarios, game config) whenever the bundle picture was uploaded or removed; only the picture and its crop are refreshed now, and the rest of the draft is left alone until Save.
- Story Bundle Play, CONVO, and GM now use the starred default connection when one is set, instead of always picking whichever connection happens to be first in the list.
- Story Bundle Play's lorebook/agent activation now goes through the same chat-metadata update path (cache merge and rollback included) the rest of the app uses, instead of separate raw requests that bypassed it.
- Story Bundle preset export no longer carries a preset's `parameters` or `systemKey` fields into the exported `.storybundle`, matching the repo's own preset exporter — neither field was ever read back on import, so this only stops writing dead data that didn't need to leave the machine.
- Fixed a Roleplay/Game preset-variables prompt staying visible after switching to a different chat while it was open; it now only shows for the chat it was actually queued for.
- Hardened the Story Bundle description's HTML sanitizer to match the same protections the in-chat HTML sanitizer already has (no `data-*` attribute pass-through, no unknown URL protocols, explicit denial of inline event-handler attributes).
- Fixed a legacy `.marinara.json` story bundle import trusting an embedded bundle or scenario picture's `imagePath` from the exporting machine at face value; it's now only kept when it resolves to a real file already in this machine's own story-bundle image store, closing off an untrusted remote image URL being rendered on import.
