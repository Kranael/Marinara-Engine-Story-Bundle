...
- Added a working CONVO button to Story Bundles (editor header, gallery detail card, and gallery cards): it starts a Conversation chat using the bundle's persona (or stays anonymous if none is set), first connection, and preset directly, activates the bundle's lorebooks, and reuses the existing Conversation setup wizard — restricted to the bundle's own characters — for picking who to message and configuring Automation, exactly like starting a Conversation normally.
