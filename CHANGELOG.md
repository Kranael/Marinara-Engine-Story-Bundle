...
- Added a "Surprise Me" option to the Story Bundle scenario picker: a static first card with a dark gradient, a dice icon, and a "Default" badge that asks the AI to invent an immersive opening hook from the active characters and setting. Dismissing the scenario dialog now resolves to Surprise Me instead of starting with no opening at all. GM's "Start Adventure" flow now shows the same scenario dialog (persona picker first, then scenario) for bundles with saved scenarios, steering the first GM turn toward the chosen saved or custom scenario instead of always improvising.
