...
### Fixed
- Story Bundle characters assigned as NPCs (not party members) are no longer discarded when Story Bundle "Start Adventure" bootstraps a new Game Mode session: they're now carried into the game as library-linked tracked NPCs, and the GM prompt injects their full character card (personality, backstory, appearance, and any custom system instructions) in a new `<known_npcs>` block — with an explicit instruction that they are not party members and shouldn't be expected to take independent turns. Dynamically-invented NPCs the GM creates during play are unaffected and keep using the existing lightweight tracked-NPC summary.
