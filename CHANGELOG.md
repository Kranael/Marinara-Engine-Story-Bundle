...
- Added a working CONVO button to Story Bundles (editor header, gallery detail card, and gallery cards): it now starts the conversation directly via the same DirectInject pattern GM uses — creating the chat with the bundle's persona (or staying anonymous if none is set), first connection, and preset, tagging it with the bundle, and activating its lorebooks and agents — with no setup wizard involved. A bundle with one (or zero) characters starts immediately; a bundle with several opens a minimal "Who do you want to message?" picker to choose which character to start with first.
