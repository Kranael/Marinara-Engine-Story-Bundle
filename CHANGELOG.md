...
- Added the Story Bundle object: a standalone data entity with title, optional HTML description, metadata (comment, creator, version, tags, image), and character, persona, lorebook, and preset assignments plus inline intros. Managed through a right-hand panel with a full-page editor (Metadata, Description, Characters, Personas, Lorebooks, Presets, Intros tabs) and import/export as `.marinara.json` with self-contained embedded characters, personas, and lorebooks including avatars, sprites, and galleries.
